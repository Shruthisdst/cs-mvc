...
class viewHelper extends View {
    ...
    public function formatText($text = ''){
        ...
        $text = preg_replace("/<list list-type=\"bullet\">/",'<ul>',$text);
        $text = preg_replace("/<\/list>/",'</ul>', $text);
        $text = preg_replace("/<list list-type=\"ordered\">/",'<ol>',$text);
        $text = preg_replace("/<\/list>/",'</ol>',$text);
        $text = preg_replace("/<list-item>/",'<li>',$text);
        $text = preg_replace("/<\/list-item>/", '</li>',$text);
        $text = preg_replace("/<italic>/",'<i>',$text);
        $text = preg_replace("/<\/italic>/",'</i>',$text);
        $text = preg_replace("/<bold>/",'<b>',$text);
        $text = preg_replace("/<\/bold>/",'</b>',$text);
        $text = preg_replace("/<underline>/",'<u>',$text);
        $text = preg_replace("/<\/underline>/",'</u>',$text);

        return $text;
    }

    public function printInRemembrance(){

        $path = str_replace(BASE_URL, '', PRECAST_URL) . 'in-remembrance.json';
        $json = file_get_contents($path);
        $data = json_decode($json, True);

        echo '
            <h1 class="title-median">In remembrance</h1>
            <div class="recent-news-meta">
                <div class="recent-news-date">Most recent entry ' . $this->formatDate($data['names'][0]['death']) . '</div>
            </div>
            <div class="recent-news-content clearfix">
                <div class="recent-news-text clear-margins">
                    <p class="in-remembrance">
                        The Academy regrets to report the passing of:<br />';
        ...
        foreach ($data['names'] as $row) {
            ...
            echo '<span><a target="_blank" href="' . BASE_URL . 'describe/fellow/' . $row['name'] . '">' . $row['firstName'] . '&nbsp;' . $row['lastName'] . '</a></span> ';
        }
